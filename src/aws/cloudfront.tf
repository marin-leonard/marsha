...
# Distribution to stream live content
resource "aws_cloudfront_distribution" "marsha_cloudfront_live_distribution" {
  enabled         = true
  is_ipv6_enabled = true
  wait_for_deployment = false
  ...
  origin {
    ...
    origin_id   = "TEMP_ORIGIN_ID/channel"
  ...
  default_cache_behavior {
    ...
    target_origin_id = "TEMP_ORIGIN_ID/channel"

    forwarded_values {
      query_string = true
      query_string_cache_keys = ["start", "end", "m"]
