...
You will be able to see it or replace it here after it is processed.`,
  ...
  [timedTextMode.CLOSED_CAPTIONING]: {
    ...
    description: 'Title for the closed captions management in the video view.',
    id: 'components.VideoView.closedCaptions',
  },
  [timedTextMode.SUBTITLE]: {
    defaultMessage: 'Subtitles',
    description: 'Title for the subtitles management in the video view.',
    id: 'components.VideoView.subtitles',
  },
  [timedTextMode.TRANSCRIPT]: {
    defaultMessage: 'Transcripts',
    description: 'Title for the transcripts management in the video view.',
    id: 'components.VideoView.transcripts',
  },
});
...
interface TimedTextTabProps {
  video: Video;
}

const TimedTextTab = ({ video }: TimedTextTabProps) => {
  const { data, status } = useTimedTextTracks({ video: video.id });

  switch (status) {
    case 'idle':
    case 'loading':
      return (
        <Box width="large" height="medium" align="center" justify="center">
          <Spinner size="large">
            <FormattedMessage {...messages.loadingSubtitles} />
          </Spinner>
        </Box>
      );
    ...
    case 'success':
      return (
        <Box width="large" align="stretch" justify="center">
          {Object.values(timedTextMode).map((mode) => (
...
const ThumbnailTab = () => {
  return <span>The thumbnail sub view</span>;
};
      ...
      content = (
        <Box direction="column" gap="medium">
          ...
          <Switch>
            ...
            <Route path={`${path}/subtitles`}>
              <Crumb
                title={<FormattedMessage {...messages.tabSubtitlesTitle} />}
              />
              <TimedTextTab video={data!} />
            </Route>

            <Route path={`${path}/thumbnail`}>
              <Crumb
                title={<FormattedMessage {...messages.tabThumbnailTitle} />}
              />
              <ThumbnailTab />
            </Route>
